...
final class ExtraLocal extends AbstractExtra {
	private static final External	NULL_OBJECT	= new ExtraLocal( null, null );
	
	private External				extra		= null;
	
	private final ExternalizerLeaf	leaf;
	
	ExtraLocal(final String recId, final ExternalizerLeaf leaf) {
		super( recId );
		this.leaf = leaf;
	}
	...
	private final External getExtra() {
		if (this.extra == null) {
			synchronized (this) {
				if (this.extra == null) {
					final External extra = this.leaf.getExternal( null, this.recId );
