...
final class CreatorSearchLocal implements CreationHandlerObject<AttachmentSearchLocal, String[]> {
	...
	final String[] search(final CacheL2<Object> cacheTree,
		...
		if (check1 != null) {
			Report.info(CreatorSearchLocal.OWNER, "HIT: " + limit + ", " + all + ", " + sort + ", " + dateStart + ", " + dateEnd + ", " + filter);
			return check1 == this.NULL_OBJECT
				? null
				: check1;
		}
		final AttachmentSearchLocal attachment = new AttachmentSearchLocal(searcher, limit, all, sort, dateStart, dateEnd, filter);
