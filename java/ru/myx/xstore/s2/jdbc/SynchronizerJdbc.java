...
final class SynchronizerJdbc implements ModuleSynchronizer {
	...
	void commitChange(
			...
			final Set<String> i1,
			final Set<String> e2,
			final Set<String> i2) throws SQLException {
		for (final String key : e1) {
			if (!e2.contains( key )) {
				try (final PreparedStatement ps = conn.prepareStatement( "DELETE FROM "
						+ this.server.getTnTreeSync()
						+ " WHERE lnkSrcId=? AND lnkTgtId=?" )) {
					ps.setString( 1, guid );
					ps.setString( 2, key );
					ps.execute();
				}
	...
	void commitChange(
		...
		try (final Connection conn = this.server.getStorage().nextConnection()) {
			this.commitChange( conn, guid, e1, i1, e2, i2 );
	...
	public BaseSync createChange(final String guid) {
		try (final Connection conn = this.server.getStorage().nextConnection()) {
			try (final PreparedStatement ps = conn.prepareStatement( "SELECT lnkSrcId,lnkTgtId FROM "
					+ this.server.getTnTreeSync()
					+ " WHERE lnkSrcId=? OR lnkTgtId=?",
