...
final class TreeJdbc {
	private static final Map<String, String>									REPLACEMENT_SEARCH_SORT			= TreeJdbc
																														.createSearchReplacementSort();
	
	private static final Map<String, String>									REPLACEMENT_FIELDS_NS			= TreeJdbc
																														.createReplacementFields( 0 );
	
	private static final Map<String, Function<OneCondition, OneCondition>>	REPLACEMENT_FILTER_CONDITIONS	= TreeJdbc
																														.createReplacementFilterConditions();
	...
	private static final Map<String, String>									EMPTY_NAMES						= Collections
																														.emptyMap();
	
	static final TreeJdbc														EMPTY							= new TreeJdbc( null,
																														null,
																														-1,
																														TreeJdbc.EMPTY_NAMES,
																														new TreeJdbcEntry[0] );
	...
	static void fillStats(
			final Map<Integer, Map<Integer, Map<String, Number>>> map,
	...
	private static final String getLetter(
			final Map<String, String> alphabetConversion,
			final String defaultLetter,
			final String title) {
		if (title == null) {
	...
	private static final String[] searchLocalJava(
			final boolean all,
			final long startDate,
			final long endDate,
			final TreeJdbcEntry[] entries) {
	...
	TreeJdbc(final ServerJdbc server,
			final String guid,
			final int luid,
	...
	final String[] getChildren(final BaseEntry<?> entry, final int count, final String sort) {
		if (sort == null || sort.trim().length() == 0 || "alphabet".equals( sort )) {
	...
	final String[] getChildrenListable(
			final BaseEntry<?> entry,
			final int count,
			final String sort,
			final Map<String, String> alphabetConversion,
		...
		if (sort == null || sort.trim().length() == 0 || "alphabet".equals( sort )) {
			...
			if (count == 0) {
				for (int i = 0; i < entryCount; ++i) {
					final TreeJdbcEntry current = entries[i];
					if (current.listable
							&& TreeJdbc.getLetter( alphabetConversion, defaultLetter, current.letter )
									.equals( filterLetter )) {
						contents.add( current.lnkId );
					}
				}
		...
		if (this.contentsListableHistory == null) {
			synchronized (this.entries) {
				if (this.contentsListableHistory == null) {
					...
					this.contentsListableHistory = contentsListableHistory.toArray( new String[contentsListableHistory
							.size()] );
				}
			}
		}
	...
	final Map.Entry<String, Object>[] search(
		...
		final ExecSearchProgram program = this.server.getFinder().search( this.guid, all, sortReplace == null
				? sort
	...
	final Map<Integer, Map<Integer, Map<String, Number>>> searchCalendar(
			final boolean all,
			final long timeout,
			final long dateStart,
			final long dateEnd,
			final String filter) {
		final ExecSearchCalendarProgram program = this.server.getFinder().searchCalendar( this.guid,
				all,
				dateStart,
				dateEnd,
				filter );
	...
	final String[] searchLocal(
		...
		try (final Connection conn = this.server.getStorage().nextConnection()) {
			...
			where = SyntaxQuery.filterToWhere( null,
					null,
					TreeJdbc.REPLACEMENT_FIELDS_NS,
					TreeJdbc.REPLACEMENT_FILTER_CONDITIONS,
					false,
					query );
			sql = new StringBuilder( 128 ).append( "SELECT t.lnkId FROM " ).append( this.server.getTnTree() )
					.append( " t, " ).append( this.server.getTnObjects() )
	...
	final String[] searchLocal(
		...
		try (final Connection conn = this.server.getStorage().nextConnection()) {
			...
			where = SyntaxQuery.filterToWhere( null,
					...
					false,
					query );
			sql = new StringBuilder( 128 ).append( "SELECT t.lnkId,o.objTitle FROM " ).append( this.server.getTnTree() )
					.append( " t, " ).append( this.server.getTnObjects() )
					.append( " o WHERE t.cntLnkId=? AND t.objId=o.objId" );
			if (!all) {
				...
				try (final ResultSet rs = ps.executeQuery()) {
					if (limit == 0) {
						while (rs.next()) {
							...
							if (TreeJdbc.getLetter( alphabetConversion, defaultLetter, rs.getString( 2 ) )
									.equals( filterLetter )) {
								result.add( guid );
							}
						}
	...
	final Map<String, Number> searchLocalAlphabet(
			final boolean all,
			final Map<String, String> alphabetConversion,
			final String defaultLetter,
			final String query) {
				...
				synchronized (this.entries) {
					if (this.alphabetSearchable == null) {
						...
						for (int i = 0; i < entryCount; ++i) {
							final TreeJdbcEntry current = entries[i];
							if (current.listable) {
								TreeJdbc.fillAlphabet( alphabetSearchable,
										alphabetConversion,
										defaultLetter,
										current.letter );
							}
	...
	final Map<Integer, Map<Integer, Map<String, Number>>> searchLocalCalendar(
			final boolean all,
			final long startDate,
			final long endDate,
			final String query) {
					...
					synchronized (this.entries) {
						if (this.calendarSearchable == null) {
							...
							for (int i = 0; i < entryCount; ++i) {
								final TreeJdbcEntry current = entries[i];
								if (current.searchable) {
									TreeJdbc.fillStats( calendarSearchable,
											calendar,
		...
		try (final Connection conn = this.server.getStorage().nextConnection()) {
			...
			where = SyntaxQuery.filterToWhere( null,
					...
					false,
					query );
			sql = new StringBuilder( 128 ).append( "SELECT t.lnkId,o.objCreated FROM " )
					.append( this.server.getTnTree() ).append( " t, " ).append( this.server.getTnObjects() )
					.append( " o WHERE t.cntLnkId=? AND t.objId=o.objId" );
			if (!all) {
				sql.append( " AND o.objState in (2,5)" );
			}
	...
	private final String[] searchLocalIntern(
			final BaseEntry<?> entry,
			final int limit,
