...
final class TransactionJdbc implements Transaction {
	...
	public final void create(
		...
		if (vrId == null || "*".equals( vrId )) {
			MatData.serializeCreate( this.server,
					this.conn,
					objId,
					"*",
					title,
		...
		MatChange.serialize( this.server, this.conn, 0, local
				? "create"
				: "create-global", lnkId, -1, 0 );
	}
	...
	public final void delete(final BaseLink linkJdbc, final boolean soft) throws Throwable {
		...
		if (soft) {
			...
			MatChange.serialize( this.server,
					this.conn,
					15,
					"recycle-all",
	...
	public void segregate(final String guid, final String linkedIdentityOld, final String linkedIdentityNew)
			throws Throwable {
		try (final PreparedStatement ps = this.conn
				.prepareStatement( "INSERT INTO "
						+ this.server.getTnObjects()
		...
		try (final PreparedStatement ps = this.conn.prepareStatement( "INSERT INTO "
				+ this.server.getTnExtraLink()
		...
		try (final PreparedStatement ps = this.conn.prepareStatement( "UPDATE "
				+ this.server.getTnTree()
				+ " SET objId=? WHERE lnkId=? AND objId=?" )) {
			ps.setString( 1, linkedIdentityNew );
			ps.setString( 2, guid );
			ps.setString( 3, linkedIdentityOld );
			ps.executeUpdate();
	...
	@Override
	public final void versionCreate(
			final String vrId,
		...
		MatVersion.serializeCreate( this.server,
				this.conn,
				vrId,
				vrParentId,
				vrComment,
	...
	@Override
	public final void versionStart(
			final String vrId,
			final String vrComment,
			final String objId,
			final String title,
