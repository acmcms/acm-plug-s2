...
class ChangeForEntry extends ChangeAbstract {
	...
	ChangeForEntry(final CurrentStorage storage, final EntryImpl entry) {
	...
	public void commit() {
		...
		try {
			...
			if (this.initialParentGuid != this.parentGuid && !this.initialParentGuid.equals(this.parentGuid)) {
				...
				invalidateTree = this.initialTitle != title || this.initialState != state || this.initialFolder != folder;
			}
			...
			if (doClearVersions) {
				...
				if (doStartVersions) {
					...
					transaction.versionStart(originalVersionId, "*", linkedIdentity, this.initialTitle, this.initialTypeName, this.entry.getOwner(), this.entry.getData());
				} else {
			...
			if (doUpdateObject) {
				if (!removed.baseHasKeysOwn() && !added.baseHasKeysOwn()) {
					if ((this.initialTitle == title || this.initialTitle.equals(title)) && this.initialCreated == created
							&& (this.initialTypeName == typeName || this.initialTypeName.equals(typeName)) && this.initialState == state && this.initialFolder == folder) {
						transaction.update(this.entry.getOriginalLink(), linkedIdentity);
					} else {
						transaction.update(this.entry.getOriginalLink(), linkedIdentity, targetVersionId, folder, created, state, title, typeName, this.commitLogged);
					}
				} else {
					transaction.update(this.entry.getOriginalLink(), linkedIdentity, targetVersionId, folder, created, state, title, typeName, this.commitLogged, removed, added);
				}
			} else if (doStartVersions) {
				transaction.update(
						this.entry.getOriginalLink(),
						linkedIdentity,
						targetVersionId,
						this.initialFolder,
						this.initialCreated,
			...
			if (invalidateTree) {
				...
				if (invalidator != null) {
					...
					if (!this.initialParentGuid.equals(this.parentGuid)) {
						final EntryImpl target = (EntryImpl) this.getStorageImpl().getStorage().getByGuid(this.parentGuid);
						if (target != null) {
							target.invalidateTree();
						}
					}
				}
			}
			this.entry.invalidateThis();
